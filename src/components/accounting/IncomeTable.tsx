...
interface Income {
  id: string; incomeNo: string; categoryId: string; categoryName: string; categoryType: string;
  branchId: string | null; amount: number; incomeDate: string; description: string;
  payer: string | null; paymentMethod: string | null; receiptNo: string | null;
...
export default function IncomeTable({
  incomes, branches, categories, total,
}: {
  incomes: Income[]; branches: Branch[]; categories: Category[]; total: number;
}) {
  ...
  function startEdit(e: Income) {
    setEditing(e);
    setForm({
      categoryId: e.categoryId,
      branchId: e.branchId ?? "",
      amount: e.amount,
      incomeDate: e.incomeDate.split("T")[0],
  ...
  async function voidIncome(id: string) {
    if (!confirm("Void this income entry? It will be removed from reports.")) return;
    setLoading(true);
    try {
      const res = await fetch(`/api/accounting/income?id=${id}`, { method: "DELETE" });
      if (!res.ok) {
        let msg = `Request failed (${res.status})`;
        try { const j = await res.json(); if (j?.error) msg = j.error; } catch {}
        throw new Error(msg);
      ...
      router.refresh();
    } catch (e: any) { alert("Error: " + (e?.message || "Failed")); } finally { setLoading(false); }
  }
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
              <tr>
                ...
                <th className="px-4 py-2.5 text-left">Income No</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {incomes.map((e) => (
                <tr key={e.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-500 text-xs">{new Date(e.incomeDate).toLocaleDateString("en-PH", { month: "short", day: "numeric", year: "numeric" })}</td>
                  <td className="px-4 py-3 font-mono text-xs text-gray-600">{e.incomeNo}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${e.categoryType === "CAPITAL" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}>
                      {e.categoryName}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{e.branchId ? branchMap.get(e.branchId) ?? "—" : "Head Office"}</td>
                  <td className="px-4 py-3 text-gray-700">{e.description}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{e.payer ?? "—"}</td>
                  <td className="px-4 py-3 text-right font-semibold text-green-600">{formatCurrency(e.amount)}</td>
                  <td className="px-4 py-3 text-center space-x-2 whitespace-nowrap">
                    <button onClick={() => startEdit(e)} className="text-blue-600 hover:underline text-xs font-medium">Edit</button>
                    <button onClick={() => voidIncome(e.id)} className="text-red-500 hover:underline text-xs font-medium">Void</button>
                  </td>
                </tr>
              ))}
              {incomes.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">No income entries for this period.</td></tr>
              )}
            </tbody>
            {incomes.length > 0 && (
              <tfoot className="bg-gray-50 font-bold">
                <tr>
                  ...
                  <td className="px-4 py-3 text-right text-green-600">{formatCurrency(total)}</td>
                  <td></td>
                </tr>
              </tfoot>
            )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 pt-6 pb-4 border-b flex items-start justify-between">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Edit Income</h2>
                <p className="text-xs text-gray-500 mt-0.5">{editing.incomeNo}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="date" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    value={form.incomeDate} onChange={(e) => set("incomeDate", e.target.value)} />
                </div>
